...
9. **Solve My Cube** (`#fixer`) — a real cube solver. The kid paints in all six sides (`scEntry`, per-face 3×3 with neighbor-color hints so orientation is unambiguous), it's validated, and a beginner-method solution is shown move-by-move, grouped into the same 7 steps. Two parts:
   ...
   - **3D preview** (`preview` mode, `sc3d*`/`C3D`, styled `.sc3d-*`): after entry, before solving, a **draggable pure-CSS 3D cube** built from the entered colors so the kid can spin it and catch a mis-entered sticker. Six face panels use the standard cube-face transforms and take the painted arrays **directly** (all identity — the neighbor-hinted entry order already matches; verified face-by-face that each side's stickers land on the correct edge, so don't reorder without re-verifying). `scInitCube3D` wires pointer-drag rotation + flick momentum + a gentle idle turntable via `sc3dTick`/`requestAnimationFrame`, which self-stops when the cube is gone or the `#fixer` screen isn't active. "See my cube →" (last entry face) → `scShowPreview`; "✨ Solve it!" → the same `scDoSolve`; "← Fix a side" → `scBackToEntry`. Set `window.SC3D_DEBUG=true` to label each sticker with its index.

`init` at the bottom calls the render functions once on load (`renderTimes`, `renderStep`, `renderVideos`, `renderCubes`, `newScramble`).
